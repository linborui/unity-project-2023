...
public class OrkAssasin_agent : Training_scripts
{
    ...
    public override void Initialize()
    {
        ...
        if(!isAgent) Player_weapon = GameObject.FindGameObjectWithTag("MainCamera").GetComponentInChildren<player_weapon>();
    ...
    public override void OnActionReceived(ActionBuffers vectorAction)
    {
        ...
        if(Ai_scripts.iFrame == 0){
            ...
            if (isAgent) {
                if(Agent_weapon.sweaping && action == 1)
                    AddReward(60);
            }
            else {
            ...
            if(distance <= 2 && action > 1 && action < 4)
                AddReward(30);
            if(distance < 5 && action == 4)
                AddReward(40);
